...
type Client = { id:string; salon_id:string; name:string; phone:string|null; email:string|null; notes:string|null; preferred_service_id:string|null; preferred_employee_id:string|null; recurrence_weeks:number|null }
...
function appointmentReminderMessage(clientName:string,time:string,salonName:string){ return `Olá ${clientName},\n\nRelembramos que amanhã tem marcação agendada connosco para ${time}.\n\nObrigado,\n${salonName}` }
...
export default function App(){
  ...
  return <div className="appShell">
    ...
    <main className="content">
      {view==='today' && <TodayView appointments={todayAppointments} retention={retention} lowStock={lowStock} clientBy={clientBy} serviceBy={serviceBy} memberBy={memberBy} salonName={salon?.name||'Salão'} onNew={()=>setModal({kind:'appointment'})} onClient={c=>setModal({kind:'clientDetail',client:c})} onBook={c=>setModal({kind:'appointment',presetClient:c.id})} />}
...
function TodayView({appointments,retention,lowStock,clientBy,serviceBy,memberBy,salonName,onNew,onClient,onBook}:{appointments:Appointment[];retention:any[];lowStock:Product[];clientBy:(id:string)=>Client|undefined;serviceBy:(id:string|null)=>Service|undefined;memberBy:(id:string)=>Member|undefined;salonName:string;onNew:()=>void;onClient:(c:Client)=>void;onBook:(c:Client)=>void}){
  return <><div className="dateStrip"><div><div className="muted cap">{dateLabel(todayISO()).split(',')[0]}</div><strong>{new Intl.DateTimeFormat(locale,{day:'2-digit',month:'long'}).format(new Date())}</strong></div><button className="primary small" onClick={onNew}>+ Marcação</button></div><div className="kpis"><Kpi label="Hoje" value={appointments.length} sub="marcações"/><Kpi label="A contactar" value={retention.length} sub="clientes"/><Kpi label="Stock" value={lowStock.length} sub="alertas"/></div><Section title="Próximas marcações">{appointments.length?appointments.slice(0,5).map(a=><AppointmentCard key={a.id} a={a} c={clientBy(a.client_id)} s={serviceBy(a.service_id)} m={memberBy(a.employee_id)} salonName={salonName} onClient={()=>clientBy(a.client_id)&&onClient(clientBy(a.client_id)!)} />):<Empty>Sem marcações para hoje.</Empty>}</Section><Section title="Clientes a recuperar">{retention.length?retention.slice(0,4).map((r:any)=><RetentionCard key={r.client.id} r={r} onBook={()=>onBook(r.client)}/>):<Empty>Nenhuma cliente em atraso.</Empty>}</Section></>
}
...
function AppointmentForm({item,presetClient,membership,members,services,clients,manager,onSaved,onVisit,onDelete}:{item?:Appointment;presetClient?:string;membership:Member;members:Member[];services:Service[];clients:Client[];manager:boolean;onSaved:(m:string)=>void;onVisit:(a:Appointment)=>void;onDelete:(a:Appointment)=>void}){
  const initialClientId=item?.client_id||presetClient||clients[0]?.id||''
  const initialClient=clients.find(c=>c.id===initialClientId)
  const [clientId,setClientId]=useState(initialClientId)
  const [serviceId,setServiceId]=useState(item?.service_id||initialClient?.preferred_service_id||services[0]?.id||'')
  const [employeeId,setEmployeeId]=useState(item?.employee_id||(manager?(initialClient?.preferred_employee_id||membership.id):membership.id))
  ...
  function changeClient(nextId:string){
    ...
    if(next?.preferred_service_id) setServiceId(next.preferred_service_id)
    if(manager && next?.preferred_employee_id) setEmployeeId(next.preferred_employee_id)
  }
  ...
  async function submit(e:FormEvent<HTMLFormElement>){
    ...
    const end=new Date(start.getTime()+Number(service?.duration_minutes||45)*60000)
    const payload={salon_id:membership.salon_id,client_id:clientId,service_id:serviceId||null,employee_id:manager?employeeId:membership.id,starts_at:start.toISOString(),ends_at:end.toISOString(),status:String(fd.get('status')||'pending'),notes:String(fd.get('notes')||'')||null}
    const q=item?supabase.from('appointments').update(payload).eq('id',item.id):supabase.from('appointments').insert(payload)
    const {error}=await q;setBusy(false)
    if(error)alert(error.message);else onSaved(item?'Marcação atualizada.':'Marcação criada.')
  }

  return <form className="formStack" onSubmit={submit}>
    <label>Cliente<select name="client_id" value={clientId} onChange={e=>changeClient(e.target.value)}>{clients.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
    <div className="grid2"><label>Data<input type="date" value={date} onChange={e=>setDate(e.target.value)} required/></label><label>Hora<input type="time" value={time} onChange={e=>setTime(e.target.value)} required/></label></div>
    <label>Serviço<select value={serviceId} onChange={e=>setServiceId(e.target.value)}>{services.map(s=><option key={s.id} value={s.id}>{s.name} · {s.duration_minutes} min</option>)}</select></label>
    {manager&&<label>Profissional<select name="employee_id" value={employeeId} onChange={e=>setEmployeeId(e.target.value)}>{members.map(m=><option key={m.id} value={m.id}>{m.display_name}</option>)}</select></label>}
    <label>Estado<select name="status" defaultValue={item?.status||'pending'}><option value="pending">Pendente</option><option value="confirmed">Confirmada</option><option value="completed">Concluída</option><option value="no_show">Faltou</option></select></label>
    <label>Observações<textarea name="notes" defaultValue={item?.notes||''}/></label>
    <button className="primary full" disabled={busy}>{busy?'A guardar…':'Guardar marcação'}</button>
    {item&&<div className="modalBottomActions"><button type="button" className="secondary" onClick={()=>onVisit(item)}>Concluir + histórico</button><button type="button" className="dangerButton" onClick={()=>onDelete(item)}>Cancelar marcação</button></div>}
  </form>
}

function ClientForm({item,prefill,membership,services,members,onSaved}:{item?:Client;prefill?:{name?:string;phone?:string};membership:Member;services:Service[];members:Member[];onSaved:(m:string)=>void}){
  const [busy,setBusy]=useState(false)
  async function submit(e:FormEvent<HTMLFormElement>){
    e.preventDefault();setBusy(true)
    const fd=new FormData(e.currentTarget)
    const payload={
      ...
      preferred_service_id:String(fd.get('preferred_service_id')||'')||null,
      preferred_employee_id:String(fd.get('preferred_employee_id')||'')||null,
      recurrence_weeks:Number(fd.get('recurrence_weeks')||0)||null,
      notes:String(fd.get('notes')||'')||null
    }
    const q=item?supabase.from('clients').update(payload).eq('id',item.id):supabase.from('clients').insert(payload)
    const {error}=await q;setBusy(false)
    if(error)alert(error.message);else onSaved(item?'Cliente atualizado.':'Cliente criado.')
  }
  return <form className="formStack" onSubmit={submit}>
    <label>Nome<input name="name" required defaultValue={item?.name||prefill?.name||''}/></label>
    <label>Telemóvel<input name="phone" inputMode="tel" defaultValue={item?.phone||prefill?.phone||''}/></label>
    <label>Email<input name="email" type="email" defaultValue={item?.email||''}/></label>
    <label>Profissional habitual<select name="preferred_employee_id" defaultValue={item?.preferred_employee_id||''}><option value="">Sem profissional definido</option>{members.map(m=><option key={m.id} value={m.id}>{m.display_name}</option>)}</select><small className="muted">Ao marcar a partir desta ficha, este profissional fica pré-selecionado. Pode ser alterado em cada marcação.</small></label>
    <label>Serviço habitual<select name="preferred_service_id" defaultValue={item?.preferred_service_id||services[0]?.id||''}><option value="">Sem preferência</option>{services.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></label>
    <label>Recorrência (semanas)<input name="recurrence_weeks" type="number" min="1" defaultValue={item?.recurrence_weeks||''} placeholder="Ex.: 5"/></label>
    <label>Observações<textarea name="notes" defaultValue={item?.notes||''}/></label>
    <button className="primary full" disabled={busy}>Guardar cliente</button>
  </form>
}
...
function AppointmentCard({a,c,s,m,salonName,onClient}:{a:Appointment;c?:Client;s?:Service;m?:Member;salonName:string;onClient:()=>void}){return <article className="card"><div className="cardRow"><span className="timeTag">{hhmm(a.starts_at)}</span><div className="grow"><strong>{c?.name||'Cliente'}</strong><span>{s?.name||'Serviço'} · {m?.display_name||''}</span><div className="miniActions"><button className="mini" onClick={onClient}>Ficha</button>{c?.phone&&<button className="mini dark" onClick={()=>whatsApp(c.phone,appointmentReminderMessage(c.name,hhmm(a.starts_at),salonName))}>WhatsApp</button>}</div></div></div></article>}
